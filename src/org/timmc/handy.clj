...
(defn ^{:added "1.0.0"} lexicomp
  "Compare two sequential collections of Comparable-or-nils lexicographically,
returning a positive integer if the first argument is greater than the
other argument, as in Java's Comparable. (Negative for second arg
greater, zero for equal.)  A nil argument sorts the same as the empty
collection. A nil collection member sorts before any
Object. Comparison is shortcutting (will do as little computation as
possible.)"
  [main other]
  ...
  (loop [main (seq main)
    ...
    (if (nil? main)
      ...
      (if (nil? other)
        1
        (let [cmp (.compareTo ^Comparable (first main) (first other))]
          (if (zero? cmp)
